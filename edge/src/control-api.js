...
export async function controlApiResponse(request, env, auth) {
  ...
  try {
    ...
    if (request.method === "GET" && url.pathname === "/api/v1/ops/runs") {
      return await runsPage(env, requestId, url);
    }
    if (request.method === "GET" && url.pathname === "/api/v1/ops/boards") {
      return await boardsPage(env, requestId, url);
    }
    if (request.method === "GET" && url.pathname === "/api/v1/ops/releases") {
      return await releases(env, requestId);
    }
