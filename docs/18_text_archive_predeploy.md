...
2026-09-26 수집기 운영 갱신: `REDSTM_TEXT_BODY_SOURCE=both`로 두 JSON 출처의 본문 큐를 각각 채운다. 두 출처는 하나의 5초 요청 간격과 TypeMoon 양보 창을 공유하지만 403/429/509 및 502/504 냉각은 출처별로 격리한다. 한쪽 상류가 막히면 다른 쪽의 목록·작품·본문을 계속 선택한다. 이는 과거 블랙툰 단독 canary 설정을 대체하며, 회차 ID나 본문이 같다고 자동 확정하지 않는다. 2026-09-26 16:35 KST 기준 수신 디렉터리 218개 중 수입 receipt 211개(게시 revision 2: 210개), 작품 본문 게시 항목 4,508개였다. TypeMoon 게시 잠금이 유지되는 동안 남은 수입·게시 작업은 양보한다.
...
2026-09-24 아카라이브 백필 운전 갱신: Newtomi PC에서 13,918개 Markdown 중 manifest와
본문 검증을 통과한 완료 task 13,915건을 확인했다. 한 배치 20건은 운영 canary용이었으므로
상한을 100건으로 올리되 합계 32MiB·파일당 2MiB·manifest 256KiB를 양쪽에서 독립
...
아카라이브 2만 건은 시험용 상한이므로 제거했다. 본문 큐는 *미처리 1,000건*만 유지하고
처리된 만큼 다음 작품/회차를 보충한다. 단일 요청·최소 5초 간격, 유료 대기, 403/429 냉각,
